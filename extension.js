...
(function (appAPI) {
    ...
    appAPI.ready(function ($) {
        ...
        if (!appAPI.isMatchPages("www.amazon.com/gp/buy/spc/handlers/display.html*", "www.amazon.ca/gp/buy/spc/handlers/display.html*", "www.amazon.co.uk/gp/buy/spc/handlers/display.html*", "www.amazon.de/gp/buy/spc/handlers/display.html*", "www.amazon.es/gp/buy/spc/handlers/display.html*", "www.amazon.fr/gp/buy/spc/handlers/display.html*", "www.amazon.it/gp/buy/spc/handlers/display.html*", "www.amazon.co.jp/gp/buy/spc/handlers/display.html*", "www.amazon.cn/gp/buy/spc/handlers/display.html*", "smile.amazon.com/gp/buy/spc/handlers/display.html*")) {
            return;
        }
        ...
        var injectionHTML = appAPI.resources.get("inject.html");
        ...
        var $affilafriendPopup = $("#affilafriend").find(".popup");
        ...
        $affilafriendPopup.find("#linkForm").submit(function (event) {
            ...
            var rawUserInput = event.target.affilafriend_link_input.value;
            var decodedUserInput = undefined;
            try {
             ...
             */
            var tagExpression = /(?:[&?]tag=)([-\w]+-\d+)(?=&|$)/;
            //See if the user-supplied text contains an affiliate tag
            var searchResult = tagExpression.exec(decodedUserInput);
            //If nothing is found, complain about it and bail out
            if (!searchResult) {
                alert("The supplied string doesn't seem to contain a valid affiliate tag");
                //TODO: a styled in-page message
            }
            //Otherwise, grab the actual ID from the results
            var foundID = searchResult[1];
            ...
            var replaceString = "?tag=" + foundID + "&"; //The '&' may end up being superfluous,
            // but doesn't hurt anything

            //Get a new URL, with the specified ID inserted all nice-like
            var taggedURL = currentURL.replace(insertExpression, replaceString);
            ...
            var successMessage = "Affiliate ID " + foundID + " applied successfully!"; //DEBUG
        });
    });
})(appAPI);
